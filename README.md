...
## 📋 装完之后：6 个配置任务（约 20 分钟）

注册平台 Key → 对接智能体 → 对接看图助手 → 完善画像 → 定制规则 → **对接 GitHub 备份**
每项的「为什么 + 步骤」：[docs/03_配置任务清单.md](docs/03_配置任务清单.md)（也可以对 AI 说「初始化」，让它带你做）

> 💡 **「初始化 3 任务」vs「配置 6 任务」的对应**：对 AI 说「初始化」后它带你做 **3 个任务**（①完善画像 ②定制规则 ③搭建看图助手）；再算上**注册平台 + 填 Key + 对接 GitHub 备份**（任务 1、2、6），正好是配置清单的 **6 个任务**：
> `6 任务 = 3 个"账号与钥匙"（注册 + 对接 + GitHub 备份） + 3 个"初始化任务"（画像 + 规则 + 看图）`
...
| `docs/03_配置任务清单.md` | ⭐ 安装后必做 6 任务（含对接 GitHub 备份，原因+步骤） |
